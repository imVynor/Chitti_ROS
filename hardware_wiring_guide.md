...
You can use any standard GPIO pins, but `pigpio` prefers pins that don't conflict with I2C/Serial. Here is the layout we initially put as a placeholder in your software:
...
3.  **TX (Transmit):** Connect to Pi **RX** (GPIO 15 / UART0 RX).
4.  **RX (Receive):** Connect to Pi **TX** (GPIO 14 / UART0 TX).
    *(Note: TX goes to RX, and RX goes to TX!)*

### Software Sync
*   You must enable the Serial Port on your Pi. Run `sudo raspi-config` -> Interfacing Options -> Serial -> *No* to login shell, *Yes* to hardware port.
*   Once plugged in, your GPS will likely appear as `/dev/serial0` on the Pi. You need to update `sensors.launch.py` to use `{'port': '/dev/serial0'}` instead of `ttyUSB0`.
